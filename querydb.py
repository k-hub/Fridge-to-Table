...
# Even though a default paramater is set, the diet still needs to be explicitly passed in as an argument.
def query_recipes_by_diet(diet='any', *ingredients):
...
def _query_recipes_with_ingredient(ingredient):
    """Query recipes in the db that have the input ingredient. Only works for one ingredient."""

    # Call _query_ingredients and pass in input ingredient to get ingredient object.
    ingred = _query_ingredient(ingredient)

    # If there is an ingredient object matching the input ingredient,
    # then get list of recipe objects that contain ingredient.
    # Query db using .recipes attr to get list of recipe objects containing ingredient.
    if ingred:
        recipes = ingred.recipes
        return recipes


def _query_recipes_for_info(unique_recipes):
...
def _query_recipes_by_diet(diet="any"):
...
##########################################################################
# Function can take more than one parameter with *. Since user can input n ingredients, * was used.
# def query_ingredients(*ingredients):
